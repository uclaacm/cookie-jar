...
import { getUserCookies } from '../utils/api';
...
const Menu: React.FC = () => {
  ...
  const [userPurchasedCookies, setUserPurchasedCookies] = useState<string[]>([]);

  useEffect(() => {
    // Fetch user's purchased cookies on component mount
    const fetchUserCookies = async () => {
      try {
        const cookies = await getUserCookies();
        if (cookies) {
          const cookieNames = cookies.map((cookie: any) => cookie.cookieName);
          setUserPurchasedCookies(cookieNames);
        }
      } catch (error) {
  ...
  const handlePurchaseSuccess = (remainingPoints: number) => {
    console.log('Purchase successful! Remaining points:', remainingPoints);
    // Refresh the user's purchased cookies after a successful purchase
    const fetchUserCookies = async () => {
      try {
        const cookies = await getUserCookies();
        if (cookies) {
    ...
    <div className="cookie-grid">
      {cookies.map((cookie, index) => (
        <CookieCard
          ...
          userPurchasedCookies={userPurchasedCookies}
